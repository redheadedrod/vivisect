...
class VQVivAllFuncgraphCanvas(vq_memory.VivCanvasBase):
    paintUp = pyqtSignal()
    ...
    def renderMemory(self, va, size, rend=None):
        ...
        tagname = 'funcblock_%.8x' % va
        hashtagname = "#" + tagname
        elem = frame.findFirstElement(hashtagname)
        if elem.isNull():
            # Lets add a codeblock element for this
            canvelem.appendInside('<div class="codeblock" id="%s"></div>' % tagname)

        self._canv_rendtagid = hashtagname
...
class VQVivAllFuncgraphView(vq_hotkey.HotKeyMixin, e_qt_memory.EnviNavMixin, QtGui.QWidget, vq_save.SaveableWidget, viv_base.VivEventCore):
    ...
    def __init__(self, vw, vwqgui):
        ...
        self.setEnviNavName('AllFuncGraph%d' % self.viewidx.next())
        ...
        self.mem_canvas = VQVivAllFuncgraphCanvas(vw, syms=vw, parent=self)
    ...
    def renderFunctionGraph(self, fvas, graph=None):
      ...
      try:
        ...
        if graph == None:
            ...
            graph = vg_graph.HierGraph(merge=self.vw._call_graph)


            print "pre: graph nodes: %d \t edges: %d %d %d" % (len(graph.nodes), len(graph.edges), len(graph.edge_by_from), len(graph.edge_by_to))
            # make "routeable" and designate some root nodes
            for node in graph.getNodes():
                # necessary for routing algorithm to know how to deal with VA's
                graph.setNodeProp(node, "cbva", node[0])
                graph.setNodeProp(node, "cbsize", 1)

            for node in graph.getNodes():
                # assign root nodes as any node with no refs to it (other than itself)
                refsto = graph.getRefsTo(node)
                reflen = len(refsto)
                if not reflen or (reflen == 1 and refsto[0][1] == node[0]):
                    graph.setHierRootNode(node)

            for fva in fvas:
                viv_graph.preRouteGraphUp(graph, fva)

            viv_graph.reduceGraph(graph, ('down',))
            ebt = 0
            for k,v in graph.edge_by_to.items():
                ebt += len(v)
            ebf = 0
            for k,v in graph.edge_by_from.items():
                ebf += len(v)
            print "reduced: graph nodes: %d \t edges: %d %d %d" % (len(graph.nodes), len(graph.edges), ebf, ebt)
            rns = graph.nodeprops.get('rootnode')
            print "graph roots: %s %s" % (len(rns), rns)

        if len(graph.nodes) > 1000:
            print "Refusing to render AllFuncGraph with >1000 nodes.  Reduce graph size."
            return
        ...
        # Update graph nodes to include width/height in "size" property
        for nid,nprops in self.graph.getNodes():
            cbva = nid

            cbname = 'funcblock_%.8x' % cbva
        ...
        # Grab and Move nodes into position
        for nid,nprops in self.graph.getNodes():
    ...
    def _renderMemory(self):
        ...
        for item in expr.split(','):
            try:
                addr = self.vw.parseExpression(item)
            except Exception, e:
    ...
    def _hotkey_paintUp(self, va=None):
        ...
        '''
        graph = viv_graphutil.buildFunctionGraph(self.vw, self.fvas, revloop=True)
...
#@idlethread
#def showFunctionGraph(fvas, vw, vwqgui):
    #view = VQVivFuncgraphView(fvas, vw, vwqgui)
    #view.show()
